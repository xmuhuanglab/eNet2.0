...
message('-------------- Calculating peak-peak co-accessibility for each stages --------------')
stages <- strsplit(stage, split = ' ')[[1]]
# load metadata
metadata <- read.csv(paste0(inputdir, metadata.file))
for(i in stages){
  # extract cells from each stage
  cells <- metadata[metadata$stage==i,]$cell
  # read in matrix data 
  ...
  # saveRDS(input_cds, file = paste0(dir,task,'/out/1.',i,'.input_cds.RDS'))
  ...
  save(conns, file = paste0(dir,task,'/out/1.',i,'.conns.Rdata'))
}
